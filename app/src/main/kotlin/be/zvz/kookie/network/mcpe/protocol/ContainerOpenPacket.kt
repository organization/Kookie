...
@ProtocolIdentify(ProtocolInfo.IDS.CONTAINER_OPEN_PACKET)
	...
	var entityUniqueId: Int = -1
	 ...
	 static fun blockInv(windowId: Int, windowType: Int, x: Int, y: Int, z: Int) : self{
		result = new self
	 ...
	 static fun blockInvVec3(windowId: Int, windowType: Int, vector3: Vector3) : self{
		return blockInv(windowId, windowType, vector3->getFloorX(), vector3->getFloorY(), vector3->getFloorZ())
	}

	 static fun entityInv(windowId: Int, windowType: Int, entityUniqueId: Int) : self{
		result = new self
		result.windowId = windowId
		result.type = windowType
