...
@ProtocolIdentify(ProtocolInfo.IDS.SET_TITLE_PACKET)
	...
	override fun encodePayload(output: PacketSerializer) {
	 ...
	 static fun type(type: Int) : self{
		result = new self
		result.type = type
		return result
	}

	 static fun text(type: Int, text: string) : self{
		result = type(type)
		result.text = text
		return result
	}

	 static fun title(text: string) : self{
		return text(TYPE_SET_TITLE, text)
	}

	 static fun subtitle(text: string) : self{
		return text(TYPE_SET_SUBTITLE, text)
	}

	 static fun actionBarMessage(text: string) : self{
		return text(TYPE_SET_ACTIONBAR_MESSAGE, text)
	}

	 static fun clearTitle() : self{
		return type(TYPE_CLEAR_TITLE)
	}

	 static fun resetTitleOptions() : self{
		return type(TYPE_RESET_TITLE)
	}

	 static fun setAnimationTimes(fadeIn: Int, stay: Int, fadeOut: Int) : self{
