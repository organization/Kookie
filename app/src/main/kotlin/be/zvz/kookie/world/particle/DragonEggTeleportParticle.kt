...
class DragonEggTeleportParticle(val xDiff: Int, val yDiff: Int, val zDiff: Int) : Particle {
    init {
    ...
    override fun encode(pos: Vector3): List<ClientboundPacket> {
        val data = (if (zDiff < 0) 1 shl 26 else 0) or
            (if (yDiff < 0) 1 shl 25 else 0) or
            (if (xDiff < 0) 1 shl 24 else 0) or
            (abs(xDiff) shl 16) or
            (abs(yDiff) shl 8) or
            abs(zDiff)
        return listOf(LevelEventPacket.create(LevelEventPacket.EVENT_PARTICLE_DRAGON_EGG_TELEPORT, data, pos))
    }
}
